...
public class MobilityBudgetEventHandler implements PersonDepartureEventHandler {
    ...
    public void handleEvent(PersonDepartureEvent personDepartureEvent) {
        log.info("Handeling PersonDepature Event");
        Id<Person> personId = personDepartureEvent.getPersonId();
        if (RunBerlinScenario.personsWithMobilityBudget.containsKey(personId)) {
            ...
            if (personDepartureEvent.getLegMode().equals(TransportMode.car)) {
                ...
                RunBerlinScenario.personsWithMobilityBudget.replace(personId, 0.0);
            ...
            if (!personDepartureEvent.getLegMode().equals(TransportMode.car) && !personGotMobilityBudget.contains(personId) && !personUsedCar.contains(personId)) {
                RunBerlinScenario.personsWithMobilityBudget.replace(personId, calculateMobilityBudget(personId));
                log.info("Person: " + personId + "MobilityBudget" + RunBerlinScenario.personsWithMobilityBudget.get(personId));
